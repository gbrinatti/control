...
ReadTimeLimit    = 1 # [s] Límite de la adquisición, en tiempo
...
WriteTimeLimit    = 1 # [s] Límite de la escritura, en tiempo
...
def read(in_data, frame_count, time_info, status):
    ...
    if ReadCounter == 0:
        ReadStartTime = time_info['current_time']
    ...
    if ReadCounter >= ReadCounterLimit:
        ReadEndTime = time_info['current_time']
...
def write(in_data, frame_count, time_info, status):
    ...
    if WriteCounter == 0:
        WriteStartTime = time_info['current_time']
    ...
    if WriteCounter >= WriteCounterLimit:
        WriteEndTime = time_info['current_time']
